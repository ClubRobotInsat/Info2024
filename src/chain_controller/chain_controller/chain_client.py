...
class ChainClientNode(Node):
    ...
    def send_goal(self, action_id:int):
        
        self.chain_client.wait_for_server()
        goal = ControlChain.Goal()
        goal.chain_action_id = action_id
        self.get_logger().info(f"Sending goal {action_id}")
...
def main(args=None):
    ...
    chain_client.send_goal(action_request)
    chain_client.send_goal(action_request+1)
