...
class Homologation(Node):
    ...
    def __init__(self):
        ...
        self.instructions = [
            # (0,0.0, 0.30, 1.7),  # Go front 80cm
            # (0,0.0, 0.0, 0.2),  # stop
            # (0,-0.30, 0.0, 5.5),

            (1, math.pi / 4.0, 1.0), # turn 45 degrees
    ...
    def execute_next_instruction(self):
        if self.sequence_index < len(self.instructions):
            ...
            if self.allowed_moving:
                ...
                match self.instructions[self.sequence_index][0]:
                    case 0:
                        instruction = self.instructions[self.sequence_index][1:]
    ...
    def turn(self, angle=2*math.pi, speed=1.0):
        twist = Twist()
        twist.linear.z = speed
        duration = angle / speed
        self.vel_publisher.publish(twist)
        self.get_logger().info(f"Robot should be turning at speed: w={speed} rad/s for {duration} s.")
